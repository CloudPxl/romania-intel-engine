...
modals_path = os.path.join(FRONTEND, "components", "EnterpriseModals.tsx")

modals_content = """\"use client\";
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/85 p-4 backdrop-blur-md">
      <div className="relative w-full max-w-4xl rounded-2xl border border-cyan-800/60 bg-[#0b111e] p-6 shadow-2xl text-white max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center mb-6 border-b border-[#1e293b] pb-4">
          <div>
            <h2 className="text-2xl font-bold tracking-tight text-cyan-400">Activare Abonament & Factura Proforma</h2>
            <p className="text-xs text-slate-400">Generare instantanee Factura Proforma pentru plata prin Ordin de Plata (OP) sau Card.</p>
          </div>
        ...
        {!proformaData ? (
          <div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              ...
              >
                <div>
                  ...
                  <p className="text-2xl font-extrabold text-white mb-3">499 <span className="text-xs font-normal text-slate-400">RON / luna</span></p>
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-2xl font-extrabold text-white mb-3">1499 <span className="text-xs font-normal text-slate-400">RON / luna</span></p>
                  <ul className="space-y-1.5 text-xs text-slate-300">
            ...
            {selectedPlan && (
              <div className="rounded-xl border border-[#1e293b] bg-[#131d2e] p-4 text-xs space-y-3">
                <span className="font-bold text-cyan-300 block uppercase text-[11px]">Date Facturare Companie (Pentru Factura Proforma):</span>
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    <label className="block text-slate-400 mb-1">Adresa Sediu Social</label>
                ...
                >
                  {loading ? "Se emite proforma..." : `Genereaza Factura Proforma (${selectedPlan === "plan_founder_vip" ? "1499" : "499"} RON)`}
                </button>
              </div>
            )}
          </div>
          ...
          <div className="space-y-4 text-xs">
            <div className="rounded-xl border border-emerald-500/40 bg-emerald-950/20 p-4 text-center">
              <span className="text-emerald-400 font-bold block text-sm">✓ Factura Proforma {proformaData.invoice_number} a fost emisa cu succes!</span>
              <p className="text-slate-300 text-xs mt-1">Total de plata: <b>{proformaData.total_ron} RON</b> pentru {proformaData.plan_name}</p>
            </div>

            <div className="rounded-xl border border-slate-800 bg-[#131d2e] p-4 space-y-2">
              <span className="font-bold text-cyan-400 block">Date Transfer Bancar (Ordin de Plata - OP):</span>
              ...
              <p className="text-slate-300">Detalii Plata: <b>{proformaData.bank_details.payment_details_prefix}{proformaData.invoice_number} ({proformaData.cui_fiscal})</b></p>
            </div>
...
export function CaietScannerModal({ isOpen, onClose, defaultTitle }: { isOpen: boolean; onClose: () => void; defaultTitle: string }) {
  const [text, setT [file, setFile] = useState<File | null>(null);
        ...
        <div className="rounded-xl border-2 border-dashed border-slate-700 bg-[#131d2e] p-4 text-center mb-3">
          ...
          <label htmlFor="caiet-upload" className="cursor-pointer block">
            <span className="text-cyan-400 font-bold block text-xs">
              {file ? `Fisier selectat: ${file.name}` : "📂 Trageti fisierul PDF sau DOCX aici (sau click pentru a alege)"}
            </span>
            <span className="text-[10px] text-slate-500 mt-1 block">Suporta Caiete de Sarcini oficiale PDF, DOCX</span>
          </label>
        </div>

        <div className="text-center text-[10px] text-slate-500 mb-2">SAU LIPITI TEXTUL DIRECT</div>

        <textarea
          value={text}
          onChange={(e) => setText(e.target.value)}
          placeholder="Lipiti aici textul din caietul de sarcini..."
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/85 p-4 backdrop-blur-md">
      <div className="relative w-full max-w-3xl rounded-2xl border border-cyan-800/60 bg-[#0b111e] p-6 shadow-2xl text-white max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center mb-4 border-b border-[#1e293b] pb-3">
          <div>
            <h3 className="text-xl font-bold text-cyan-400">Scanner Eligibilitate Granturi & Licitatii Strategice</h3>
            <p className="text-xs text-slate-400">Evaluare automata a profilului companiei conform ghidurilor PNRR / MIPE 2026.</p>
          </div>
          <button onClick={onClose} className="text-slate-400 hover:text-white">✕</button>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 text-xs mb-4">
          ...
          <div>
            <label className="block text-slate-400 mb-1">Cifra de Afaceri Anuala (RON)</label>
            <input
              type="number"
              value={turnover}
        ...
        {result && (
          <div className="mt-4 space-y-3 rounded-xl border border-slate-800 bg-[#131d2e] p-4 text-xs">
            ...
            <div className="space-y-2 mt-2">
              <span className="font-bold text-slate-400 uppercase text-[10px]">Linii de Finantare Eligibile:</span>
              {result.matched_grants.map((g: any, i: number) => (
                <div key={i} className="rounded bg-black/40 p-3 border-l-2 border-cyan-500">
                  <div className="flex justify-between">
                    ...
                    <span className="font-bold text-emerald-400">Pana la {g.eligible_grant_up_to}</span>
                  </div>
                  <p className="text-slate-400 text-[11px] mt-1">Cofinantare: {g.required_co_financing} | Baza legala: {g.legal_basis}</p>
                </div>
              ))}
            </div>
          </div>
...
export function CopilotChatModal({ isOpen, onClose, tenantId, report72h }: { isOpen: boolean; onClose: () => void; tenantId: string; report72h: any }) {
  const [messages, setMessages] = useState<{ sender: "user" | "ai"; text: string }[]>([
    { sender: "ai", text: "Buna ziua! Sunt Copilotul AI RO-INTEL. Cum va pot ajuta cu strategiile de ofertare, cerintele tehnice sau dosarele din ultimele 72 de ore?" }
  ]);
      ...
      <div className="relative w-full max-w-3xl rounded-2xl border border-cyan-800/60 bg-[#0b111e] p-6 shadow-2xl text-white flex flex-col h-[85vh]">
        ...
        {report72h && (
          <div className="rounded-xl bg-[#131d2e] p-3 text-xs mb-3 border border-slate-800 space-y-1">
            <span className="font-bold text-slate-300 block">Sinteza Macro Ultimele 72h:</span>
        ...
        <div className="flex-1 overflow-y-auto space-y-3 p-2 text-xs">
          ...
          {loading && <div className="text-slate-400 text-xs animate-pulse">Copilotul AI analizeaza dosarele pre-SEAP...</div>}
        </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm">
      <div className="relative w-full max-w-xl rounded-2xl border border-[#1e293b] bg-[#0b111e] p-6 shadow-2xl text-white">
        <div className="flex justify-between items-center mb-4 border-b border-[#1e293b] pb-3">
          <h3 className="text-xl font-bold text-emerald-400">Simulator Sanse de Castig & Marja Optima</h3>
          <button onClick={onClose} className="text-slate-400 hover:text-white">✕</button>
        </div>
        <div className="space-y-3 text-xs">
          <div>
            <label className="block text-slate-400 mb-1">Buget Estimat Autoritate Contractanta (RON)</label>
          ...
          <div>
            <label className="block text-slate-400 mb-1">Pret Ofertat Propus (RON)</label>
          ...
          {result && (
            <div className="rounded-xl border border-slate-800 bg-[#131d2e] p-4 text-center mt-3">
              <p className="uppercase text-slate-400 text-[10px]">Probabilitate Estimata de Atribuire</p>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm">
      <div className="relative w-full max-w-2xl rounded-2xl border border-[#1e293b] bg-[#0b111e] p-6 shadow-2xl text-white max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center mb-4 border-b border-[#1e293b] pb-3">
          <h3 className="text-xl font-bold text-cyan-400">Generator Solicitare Clarificari (Legea 98/2016)</h3>
          <button onClick={onClose} className="text-slate-400 hover:text-white">✕</button>
        </div>
      ...
      <div className="relative w-full max-w-5xl rounded-2xl border border-cyan-800/60 bg-[#0b111e] p-6 shadow-2xl text-white max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center mb-4 border-b border-[#1e293b] pb-3">
          <div>
            ...
            <p className="text-xs text-slate-400">Monitorizare stadiu intern: evaluare tehnica, adrese clarificari si marje estimate.</p>
          </div>
          <button onClick={onClose} className="text-slate-400 hover:text-white">✕</button>
        </div>

        {loading ? (
          <div className="flex h-48 items-center justify-center text-xs text-slate-400">Se incarca pipeline-ul companiei...</div>
          ...
          <div className="flex h-48 flex-col items-center justify-center text-xs text-slate-500 space-y-2">
            ...
            <span className="text-[11px] text-cyan-400">Deschideti orice dosar din feed-ul principal si apasati "Salveaza in Pipeline".</span>
          </div>
        ) : (
              ...
              <div key={d.deal_id} className="rounded-xl border border-[#1e293b] bg-[#131d2e] p-4 text-xs space-y-2">
                <div className="flex justify-between items-start">
                  ...
                  <div className="text-right">
                    ...
                    <span className="block text-[10px] text-emerald-400 font-bold">Marja Tinta: {d.target_margin_pct}%</span>
                  </div>
                </div>
