...
async def get_pool() -> Optional[asyncpg.Pool]:
    """Lazily creates a small connection pool against Supabase's transaction
    pooler. Returns None (rather than raising) if DATABASE_URL isn't set, so
    callers can degrade gracefully instead of crashing the whole process."""
...
async def finish_tick(tick_id: Optional[int], sources_run: int, new_count: int, errors: int) -> None:
    pool = await get_pool()
    if pool is None or tick_id is None:
        return
    async with pool.acquire() as conn:
        await conn.execute(
            """
