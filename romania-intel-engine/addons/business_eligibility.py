...
ELIGIBLE_GRANT_PROGRAMS = [
    ...
    },
    {
        ...
        "co_financing_pct": 25,
        "legal_basis": "Ghid Specific ADR Nord-Vest / ADR Centru / ADR Nord-Est"
    }
]
...
class BusinessEligibilityEngine:
    ...
    def evaluate_company(
        ...
        for p in ELIGIBLE_GRANT_PROGRAMS:
            ...
            if is_caen_match and is_turnover_match:
                matched_grants.append({
                    ...
                    "program_name": p["name"],
                    "eligible_grant_up_to": f"€{p['max_grant_eur']:,.0f}",
                    "required_co_financing": f"{p['co_financing_pct']}%",
                    "legal_basis": p["legal_basis"],
                    "eligibility_score": 9.5 if is_caen_match and is_turnover_match else 7.8,
                    "action_required": "Constituiți dosarul de finanțare și planul de afaceri tehnico-financiar conform cerințelor din Ghid."
                })
        ...
        return {
            ...
            "qualification_status": "Eligibil pentru Fonduri Nerambursabile & Licitații Strategice" if matched_grants else "Necesită Ajustare CAEN / Cifră Afaceri",
            "overall_eligibility_score": overall_score,
            "matched_programs_count": len(matched_grants),
            "matched_grants": matched_grants,
            "advisory_summary": (
                f"Compania {company_name} (CUI {cui_fiscal}) întrunește criteriile de eligibilitate pentru {len(matched_grants)} linii majore de finanțare nerambursabilă din România. "
                "Recomandăm participarea ca ofertant individual sau lider de asociere la procedurile pre-SEAP din județele prioritare."
            )
        }
